...
To begin comparing \(f(x)=\color{RoyalBlue}(2x-5)^3\) to its derivative, notice that \(f'(x)=2x^2-120x+150\) can be factorising and rearranged as follows:
...
\begin{center}
\begin{tcolorbox}[colback=red!5,width=16cm,colframe=red!70!black,height=2.2cm]
\vspace{-0.5cm}
...
\end{align*}
        ...
        \begin{minipage}{5cm}
        \begin{align*}
    \text{If }\hspace{1.4cm} f(x)&=\cos{x}\\[0.5em]
    \text{ then }\hspace{0.8cm} f'(x)&=-\sin{x}
\end{align*}
        \end{minipage}
        };
    \end{tikzpicture}
...
A slightly expanded version of these rules may be found on the Higher formula sheet. However they may be more powerfully combined with the Chain Rule for Differentiation, allowing expressions such as \(\cos{(3x-\pi)}\) and \(\sin^2{x}\) to be differentiated.
