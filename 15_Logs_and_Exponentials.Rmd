...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \end{multicols}
    \end{enumerate}
    \item Solve the equation $\log_3{x}-\log_3{5}=\log_3{2}$.
    \item Solve $\log_2{x}+\log_2{(x+7)}=3$, where $x>0$.
    ...
    \begin{center}
        ...
        \item A logarithmic function, $f$, is defined for $x>1$. The diagram shows the graph of $y=f(x)$.
        \begin{center}
        \begin{tikzpicture}[scale=0.8]
                    \draw[-stealth] (-0.5,0) -- (9,0) node [above] {$x$};
                    \draw[-stealth] (0,-2) -- (0,3) node[left] {$y$};
                    \draw[smooth,thick,domain=0.1:5.5,xshift=1cm,yshift=0cm] plot (\x,{ln(\x)/ln(3)}) node[right] {$y=f(x)$};
                    \draw[fill] (2,0) circle (0.05) node[below] {$2$};
                    \draw[fill] (4,1) circle (0.05) node[above] {$(4,1)$};
                    \draw[dashed] (1,-2) -- (1,3);
                \end{tikzpicture}
                \end{center}
                \item
