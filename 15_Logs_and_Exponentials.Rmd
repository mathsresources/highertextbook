...
Given a function $f$, defined on a suitable domain such that an inverse function exists, the graph of its inverse function $y=f^{-1}(x)$ can be obtained by \textit{reflecting the graph of} $y=f(x)$ \textit{in the line} $y=x$.
...
\begin{center}
    \begin{tikzpicture}[scale=0.9]
        ...
        \draw[smooth,thin,domain=0.1:3.5,black!50] plot (\x,{(ln(\x))/(ln(3))}) node[right] {$y=\log_3{x}$};
        \draw[black!50,fill] (1,0) circle (0.05) node[above left] {$1$};
        \draw[black!50,fill] (3,1) circle (0.05) node[above] {$(3,1)$};
        \draw[smooth,thick,domain=0.04:3.5,xshift=1cm,yshift=2cm] plot (\x,{(ln(\x))/(ln(3))}) node[right] {$y=\log_3{(x-1)}+2$};
        \draw[black,fill,xshift=-0.3cm] (2,2) circle (0.05) node[above] {$(2,2)$};
        \draw[black,fill] (4,3) circle (0.05) node[above] {$(4,3)$};
        \draw[dashed] (1,-2) -- (1,4);
        \node[below right,PineGreen] at (9.5,3) {
        \begin{minipage}{7.5cm}
        $y=\log_3{x}$: \qquad  $(1,0)$ and $(3,1)$
        
        \vspace{0.7cm}
        
        $f(x-1)+2$ translates \textit{right 1} and \textit{up 2}
        
        \vspace{0.7cm}
        
        $y=\log_3{(x-1)}+3$: \qquad $(2,2)$ and $(4,4)$
        \end{minipage}
        };
    \end{tikzpicture}
\end{center}
