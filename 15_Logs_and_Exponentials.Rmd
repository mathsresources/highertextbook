...
\begin{enumerate}
    \item[(b)] After 5 hours, only 50.7 micrograms of the substance remains.\newline Determine the value of $k$, rounded to three significant figures.
\end{enumerate}
...
k&=0.1 && \color{PineGreen}\longleftarrow \text{Calculate and round as required}k
\end{align*}

\end{tcolorbox}

A feature of exponential functions is that the time taken for the value to grow or decay by a any given proportion is always constant. For example the time taken for exponential growth to double a value from 20 to 40 will be the same time it will take to double from 50 to 100.
...
\begin{enumerate}
    \item[(c)] Find the time it will take for only half of the original substance to remain (its \textit{half-life}).
\end{enumerate}
